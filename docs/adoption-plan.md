...
Status: GitHub publishes the FastaGuard v1.0.0 source release and binaries.
Bioconda remains at v0.7.0 on `linux-64`, `linux-aarch64`, `osx-64`, and
`osx-arm64` until its v1 update merges; BioContainers publishes
`quay.io/biocontainers/fastaguard:0.7.0--hfa8f182_0` in the meantime.
...
- Keep `docs/biotools-registration.json` as a reviewable bio.tools draft until
  a maintainer submits it. It describes the v1.0.0 source release while keeping
  its Bioconda and BioContainers downloads pinned to the verified v0.7.0
  distribution. It uses EDAM `format_1929` (FASTA) with `operation_3180`
...
v1.0.0 Bioconda package has been published.
