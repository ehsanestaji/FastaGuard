...
NONHUMAN_PROVENANCE = r"(?:(?:an?\s+)?(?:automated|machine|nonhuman)(?:\s+(?:tool|system|model))?)"
PROHIBITED_ATTRIBUTION_PATTERNS = [
    re.compile(rf"(?im)^co-authored-by:\s*{NONHUMAN_PROVENANCE}\b"),
    re.compile(
        rf"(?im)^generated(?:-|\s+)by\s*:?\s*{NONHUMAN_PROVENANCE}\b"
    ),
    re.compile(rf"(?i)written with {NONHUMAN_PROVENANCE}\b"),
    re.compile(
        r"(?i)\bautomated[- ]assisted\s+(?:contribution|change|code|content|documentation)\b"
    ),
    re.compile(r"(?im)^automated\s+provenance\s*:"),
]
ATTRIBUTION_TRACE_SAMPLES = (
    "Generated-by: automated tool",
    "generated by machine",
    "Co-authored-by: nonhuman system",
    "Written with an automated model",
    "automated-assisted contribution",
...
class CommunityHealthTest(unittest.TestCase):
    ...
    def test_tracked_tree_has_no_prohibited_nonhuman_attribution_traces(self):
        violations = []
        for path in self.tracked_paths():
            if path == "tests/python/test_community_health.py":
                continue
            text = (ROOT / path).read_text(errors="ignore")
    ...
    def test_attribution_patterns_allow_release_provenance(self):
