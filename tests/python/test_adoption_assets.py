...
class AdoptionAssetsTest(unittest.TestCase):
    ...
    def test_nf_core_starter_matches_current_nf_core_module_shape(self):
        ...
        self.assertIn("fastaguard ${fasta}", main_nf)
        self.assertIn("${args}", main_nf)
        self.assertIn("--out ${prefix}.fastaguard.html", main_nf)
        self.assertIn("--json ${prefix}.fastaguard.json", main_nf)
        self.assertIn("--tsv ${prefix}.fastaguard.tsv", main_nf)
        self.assertIn("--multiqc ${prefix}.fastaguard_mqc.json", main_nf)
        self.assertEqual(main_nf.count("tuple val(meta), path(\"*.fastaguard"), 4)
        self.assertNotIn("--outdir", main_nf)
        self.assertNotIn("--threads", main_nf)
        self.assertNotIn("exit_code", main_nf)
    ...
    def test_snakemake_wrapper_has_upstream_prep_test_layout(self):
        ...
        self.assertIn('extra = snakemake.params.get("extra", "")', wrapper_py)
        self.assertIn('"{extra} "', wrapper_py)
        self.assertIn("{log}", wrapper_py)
        for output_name in ("html", "json", "tsv", "multiqc"):
            with self.subTest(output=output_name):
                self.assertIn(f"snakemake.output.{output_name}", wrapper_py)
        self.assertNotIn("--outdir", wrapper_py)
        self.assertNotIn("--threads", wrapper_py)
        self.assertNotIn("exit_code", wrapper_py)
